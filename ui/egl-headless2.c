...
static int egl_headless2_surfaceless_init(DisplayOptions *opts)
{
    ...
    if (opts->u.egl_headless2.has_index) {
        int index = opts->u.egl_headless2.index;
        EGLDeviceEXT *devices;
        EGLint ndevs;

        info_report("egl: using specified index: %d", index);

        if (!egl_headless2_can_use_enumeration()) {
            error_report("egl: either EGL_EXT_platform_base or EGL_EXT_device_enumeration is missing");
            return -1;
        }

        b = eglQueryDevicesEXT(0, NULL, &ndevs);
        if (b == EGL_FALSE) {
        ...
        devices = g_malloc(sizeof(EGLDeviceEXT) * ndevs);
        b = eglQueryDevicesEXT(ndevs, devices, &ndevs);
        if (b == EGL_FALSE) {
        ...
        if (index < 0 || index >= ndevs) {
            error_report("egl: device index is out of allowed ranged (Max. devices: %d)", ndevs);
            g_free(devices);
            return -1;
        }
        ...
        info_report("egl: using default display");
...
static QemuDisplay qemu_display_egl = {
    .type       = DISPLAY_TYPE_EGL_HEADLESS2,
